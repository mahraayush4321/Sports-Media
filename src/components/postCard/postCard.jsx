...
const PostCard = ({ postData }) => {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://s1backend1.onrender.com/api/v1/updatePost/${postData._id}`,
        {
          method: "PUT",
          headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      // eslint-disable-next-line react/prop-types
      const response = await fetch(
        `https://s1backend1.onrender.com/api/v1/deletePost/${postData._id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `${token}`,
    ...
    >
      ...
      <Modal isOpen={isPopupOpen} onClose={closePopup}>
        ...
        <ModalContent>
          ...
          <ModalFooter>
            <Button colorScheme="blue" mr={3} onClick={openEditModal}>
              Edit Post
            </Button>
